...
describe('Estagio page', () => {
  ...
  it('Shoud be able to apply for a new internship', async () => {
    const { getByTestId, getByPlaceholderText } = render(<Estagio />);
    ...
    act(() => {
      ...
      fireEvent.change(inputEmail, {
        target: { value: 'maxdickinson@gmail.com' },
      });
    });

    //No trecho acima definimos os valores para cada um dos inputs.

    const formSubmit = await waitForElement(() => getByTestId('form-field'));
    ...
    expect(window.localStorage.setItem).toHaveBeenCalledWith(
      ...
      JSON.stringify({
        id: '1',
        type: 'Estágio I',
        name: 'Emanuel Maximiliano',
        email: 'maxdickinson@gmail.com',
        matricula: '404513',
        status: 'pending',
      }),
    );
